...
def extract_quality_from_bed_sequential(fastq_folder, bed_pattern, output_dir):
    # Create a dictionary to store FASTQ records indexed by the part of the FASTQ filename before "_qual_"
    fastq_records_dict = {}
    for fastq_file in glob.glob(os.path.join(fastq_folder, '*_qual_*.fastq')):
        base_name = os.path.splitext(os.path.basename(fastq_file))[0]
        prefix = base_name.split('_qual_')[0]
